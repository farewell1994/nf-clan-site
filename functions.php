...
function getClanTable($clan)
{
    ...
    $response = unserialize(file_get_contents("http://api.neverfate.ru/sostav.php?cl=" . $clan));
    foreach ($response as $char) {
        ...
        $result .= "<tr>";
        $result .= "<td><a target='_blank' href='http://neverfate.ru/inf.php?cid=" . $char["cid"] . "'>" . sprintf("%s [%s]", $login, $char["level"]) . "</a></td><td>" . $status . "</td><td>" . iconv('CP1251', 'UTF-8', $char["room"]) . "</td>";
        $result .= "</tr>";
    }
    $result .= "</table>";
